(ns kura.kotoba-oracle
  "Runs the shipped decision cores.

  `kotoba/*.kotoba` holds the decisions; `resources/kura/oracle/*.kir.edn` is
  what was compiled from them and what ships. This namespace is the seam, and
  it is deliberately thin: it resolves a resource, executes an export, and
  decides nothing.

  ## Why this exists

  `order_core.kotoba` landed under `kura.kotoba-decision-parity-test`, which
  ran both implementations over the same inputs and required the same answers.
  That was the right first step and it is still here. But two implementations
  bound by a test are still two implementations, and the measure of a port is
  not how many host lines went away; it is whether the AUTHORITY moved
  (ADR-2608112100). Until now it had not: the `.kotoba` was a checked replica
  and the `.cljc` was what ran. Now the `.kotoba` is what runs for the inputs
  it can express, and the `.cljc` keeps the halves that are not decisions —
  reading a map, naming a key, collecting reasons.
  ...
  neither is what a `kura.order` map holds. `i64` and `i64-value` are that
  conversion, kept here so a host never has to know which runtime it is on.
  `order_core.kotoba` has no `:bool` in it at all — every predicate there
  returns `:i64` 1/0 — so `truth` is the other conversion, and it is spelled
  out rather than left to `boolean`, which would read `0` as true.
  ...
  shipped. Which path answers a call is decided by the DATA (see
  `kura.order`'s boundary note), never by whether the artifact loaded.
  ...
  see the boundary note on `kura.order`, and `kura.cljs-runner` for the one
  line a node host needs."
...
(def cores
  ...
  `kura_core` and `placement_core` are not here. They are still held to the
  `.cljc` by parity alone: this slice moved the authority for the order
  decisions only, and a map that listed a core nobody executes would say the
  opposite of what it means."
  {:order "kotoba/order_core.kotoba"})

(defn resource-path [id]
  (str "kura/oracle/" (name id) ".kir.edn"))
...
(defn call
  ...
  see `record`, `i64`, `i64-value` and `truth` for the conversions."
  [id export args]
...
(defn truth
  "Guest `:i64` 1/0 -> host boolean.

  `order_core.kotoba` says why it is 1/0 and not `:bool`: `string=?` returns
  `:i64`, so every predicate built on it does too. `(pos? …)` rather than
  `boolean`, because in ClojureScript a `js/BigInt` 0 is truthy and a host that
  wrote `boolean` would read every rejection as an acceptance."
  [n]
  (pos? (i64-value n)))
